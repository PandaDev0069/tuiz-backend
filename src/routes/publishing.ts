...
async function validateQuizForPublishing(quizId: string): Promise<{
  ...
  try {
    ...
    if (quizError) {
      ...
      return { isValid: false, errors: ['Failed to fetch quiz'], warnings: [] };
    }
    ...
    if (questionError) {
      ...
      return { isValid: false, errors: ['Failed to count questions'], warnings: [] };
    }
    ...
    if (questionCount && questionCount > 0) {
      ...
      if (questionsError) {
        ...
        return { isValid: false, errors: ['Failed to fetch questions'], warnings: [] };
      }
      ...
      for (const question of questions || []) {
        ...
        if (answerError) {
          logger.error({ error: answerError, questionId: question.id }, 'Error counting answers');
          errors.push(`Failed to count answers for question: ${question.question_text}`);
          continue;
        }

        if (!answerCount || answerCount === 0) {
          errors.push(`Question "${question.question_text}" must have at least one answer`);
        } else {
          ...
          if (answersError) {
            ...
            errors.push(`Failed to fetch answers for question: ${question.question_text}`);
            continue;
          }
          ...
          if (correctAnswers.length === 0) {
            errors.push(
              `Question "${question.question_text}" must have at least one correct answer`,
            );
          } else if (correctAnswers.length > 1) {
            errors.push(
              `Question "${question.question_text}" must have exactly one correct answer`,
            );
          }
        }
      }
